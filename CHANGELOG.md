...
- **AFK Screensaver & Idle Timer Thresholds**:
  - **Screensaver Overlay Timeout**: Configured to launch automatically after **1 minute** of user inactivity.
  - **Focus Timer Auto-Pause**: Configured to pause active focus session after **3 minutes** of system idle time.
  - **Settings Controls**: Added configurable input controls for Screensaver and AFK Idle Pause timeouts inside the Settings drawer.
- **Scheduled Tasks View (`📅 Scheduled (All Days)`)**: Added a dedicated workspace filter pill to view all scheduled tasks across any day of the week.
