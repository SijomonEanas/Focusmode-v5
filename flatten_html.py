...
# Add ID to AI Review Card so we can hide it
content = content.replace('<div class="card glass-card ai-review-card">', '<div id="ai-review-card-container" class="card glass-card ai-review-card">')

# Flatten the dashboard HTML
...
# Replace the existing dashboard section entirely
content = re.sub(r'<section id="dashboard-section".*?</section>', dashboard_html, content, flags=re.DOTALL)

# Ensure the new dashboard section is INSIDE the tasks-section (before its closing tag)
# First remove it from wherever it is if it got placed as a sibling
content = content.replace(dashboard_html, '')
content = content.replace('</section>\n\n      </div>\n\n      <!-- Calendar Pop-up', dashboard_html + '\n</section>\n\n      </div>\n\n      <!-- Calendar Pop-up')

with open(html_path, "w", encoding="utf-8") as f:
    f.write(content)

print("Updated HTML to flattened Grid structure.")
